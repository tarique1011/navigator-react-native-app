    ...
    renderCard() {
        if (this.state.order) {
            return (
			    ...
			    >
					<Text style={{ marginBottom: 10 }}>Order will be delivered in 30 minutes</Text>
                    <Button 
                        icon={<Ionicons name='md-locate' color='#ffffff' size={18} style={{ margin: 10 }} />}
                        title='Track Order'
                        buttonStyle={{ justifyContent: 'center', marginBottom: 10, backgroundColor: '#9e0606' }}
                    />
				</Card>
            );
        }
        ...
        return (
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{ fontSize: 20 }}>We have not received any order from you</Text>
            </View>
        );
    }
